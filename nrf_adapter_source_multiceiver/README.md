...
Phase 0.5 of `multi_device_refactor_plan.md`: replaces the shipped
`nrf_adapter_source/` (STM32CubeMX + HAL + `mokhwasomssi/stm32_hal_nrf24l01p`,
single nRF24 RX pipe) with a bare-metal firmware (direct CMSIS register
access, no HAL/CubeMX) that adds real nRF24L01+ multiceiver support: each
attached device (P906, later L1060) gets its own hardware RX pipe, so the
host can attribute a response to a device by pipe number instead of by
packet content/timing.
...
- **Parity layer**: reimplements every command the shipped firmware
  ...
  **Validated live 2026-07-12**: flashed to a real STM32F030F4P6 board,
  full round trip confirmed through the actual (unmodified) GUI —
  connect, auto-match against a real P906, link, realtime chart data.
- **Pipe extension**: adds `CMD_NRF_OPEN_PIPE` (0x23, pipes 1–5 only) and
  ...
  when enabled, changes `REP_NRF_RECV_OK` (0x12) to lead with a pipe-number
  byte (from `STATUS.RX_P_NO`) before the raw 32-byte nRF24 payload.
  **Currently compiled out** (`protocol.c`: `#define PIPE_TAG_RECV_OK 0`)
  because the existing `nrf24_adapter.py`/`mdp_p906.py` read `data[0]` as the
  packet type with no knowledge of a pipe-tag prefix — turning it on breaks
  every response by shifting all payload bytes right by one (confirmed
  live: P906 replies arrived as `data[0]=0x00` "Unhandled Type-0" instead of
  `data[0]=0x05`). That's expected per the plan's staged validation —
  `bus.py` (Phase 1) is what's supposed to consume the pipe byte. Flip the
  define to 1 and reflash for a dedicated (non-GUI) two-device pipe-routing
  test; flip back to 0 before any GUI-based testing.
  **Validated live 2026-07-12** (see Validation status below).
...
- **UART**: interrupt-driven RX and TX ring buffers instead of DMA +
  ...
  latency to every nRF24 IRQ — directly on the path the refactor plan flags
  as timing-sensitive (50Hz Type-8 polling). USART1 is NVIC priority 1,
...
- **Multiceiver addressing**: `NRF_OPEN_PIPE` only accepts pipes 1–5, not 0.
  ...
  the adapter's own ACK reception. This matches the plan's note that the
  real MDP-M01 hub uses the same pipes-1–5 scheme (`base_addr + (0xE1+k)`,
  5-device cap). Per nRF24 hardware, pipes 2–5 only have single-byte RX
...
- **Pipe number source**: `STATUS.RX_P_NO`, read for free off the SPI
  ...
  transaction, so this doesn't touch the timing-sensitive 50Hz Type-8 polling
  path the plan flagged as a risk.
...
1. **Parity check — done 2026-07-12.** Real P906, real board: connect,
   auto-match, link, and realtime chart data all confirmed working through
   the unmodified GUI. (`CON-ERR` rate over a longer session not yet
   specifically compared against the shipped firmware's baseline.)
2. **Pipe extension — done 2026-07-12.** Two real devices (P906 on pipe 0,
   L1060 dispatched onto pipe 1 via `CMD_NRF_OPEN_PIPE`) on one adapter,
   pipe-tagged `REP_NRF_RECV_OK` enabled (`PIPE_TAG_RECV_OK 1`). Confirmed:
   opening pipe 1 for the L1060 doesn't disturb pipe-0 (P906) reception;
   `CMD_NRF_SET_TX_TARGET` correctly retargets outgoing sends to the L1060
   and its Type-7 reply reports `pipe=1`; retargeting back to the P906
   reports `pipe=0` again. One notable edge case observed: while `TX_ADDR`/
   `RX_ADDR_P0` is retargeted to an address that's *also* separately open on
   pipe 1 (both pipes momentarily sharing the identical address), the
   response still attributes to pipe 1, not pipe 0 — the hardware favors the
   already-open non-zero pipe over the transient P0 mirror. That's the
   better outcome for `bus.py`: routing by `RX_P_NO` will land replies on a
   device's stable pipe assignment rather than whichever address is
   transiently sitting in `TX_ADDR`.

Both validations now pass — Phase 0.5 is complete. Phase 1 (the `bus.py`
transport rewrite that consumes the pipe byte) can start its own

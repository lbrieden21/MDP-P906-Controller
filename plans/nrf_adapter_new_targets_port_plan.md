...
6. GUI, 60s two-device run against the P906 + L1060 bench, then `tools/noack_report.py` to compare sample count and no-ack rate against the recorded baselines (STM32F030 4.61%, Teensy 4.1 1.91% — `README.md:141-143`). Three things needed to read the result correctly:
   - **The no-ack residue is a bench characteristic, not adapter behaviour.** On both existing adapters it is overwhelmingly P906 (`..E2`) traffic with the L1060 (`..E3`) nearly clean. A new board landing near ~2% is not automatically worse than the Teensy 4.1.
...
~65.6 req/s — noticeably below every other board (all in the ~103–116 req/s range), despite this being the fastest core besides the Teensy 4.x. No cause has been isolated for the gap; reported as a plain measurement rather than attributed to a guess. All 7 no-acks were P906 (`..E2`) traffic, L1060 clean — the same bench-characteristic pattern seen on every other board. At ~65.6 req/s this board sits far below the ~180–185 req/s overflow onset, so zero overflows is expected, not a merit. `gui_source/settings.json` already pointed at `/dev/ttyACM0`, so no repointing was needed. The mdp.log from this run carried a prior session's entries too; `tools/noack_report.py` was run against a sliced copy (from the run's own `---- NEW SESSION ----` marker onward) to avoid conflating the two.
